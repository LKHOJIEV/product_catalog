...
//begin
//  for rec in (
//select rp.rtpl_id,pv.edpv_id,pv.def name,oev.obj_value_id tech_pack_id,'103' dcgr_id,'14' msrr_id,'Мегабайт' def,'Мб.' alias_def
//  from bis.rate_plans rp
//  left join bis.obj_extented_value oev on oev.group_id = 106 and oev.obj_id = rp.rtpl_id and oev.param_num = 5 
//   and sysdate between oev.start_date and oev.end_date and oev.obj_value_id != 5405
//  left join bis.entity_dict_values dv on (dv.entity_id = rp.rtpl_id or dv.entity_id = oev.obj_value_id)
//  left join bis.entity_dict_possible_values pv on pv.edpv_id = dv.edpv_edpv_id and pv.del_date is null  
// where rp.end_date > sysdate and rp.recurring_flag is not null
//   and pv.edpv_id is not null
//) loop
//dbms_output.enable(10000000);
//
//dbms_output.put_line('
//merge (a:Characteristics_details{code:"rtpl",type:"psrf_cos",rtpl_id:"'||rec.rtpl_id||'",definition:"'||rec.name||'"})
//      set a.tech_pack_id="'||rec.tech_pack_id||'",a.priority="",a.discount_group="'||rec.dcgr_id||'",a.measure_unit="'||rec.def||'",a.definition= '''||rec.name||''',a.cos_id= "'||rec.edpv_id||'",a.alias_definition="'||rec.alias_def||'"
//merge	(b:Characteristics{definition:"BundleCharacteristics",type:"call_credit",rtpl_id:"'||rec.rtpl_id||'"})
